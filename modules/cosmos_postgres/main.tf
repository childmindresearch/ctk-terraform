resource "azurerm_cosmosdb_postgresql_cluster" "cluster" {
  ...
  coordinator_public_ip_access_enabled = true // Required for admin login to the DB from home wifis.
...
resource "azurerm_private_endpoint" "cosmos_postgres" {
  ...
  private_service_connection {
    name                           = format("psc-cosmos-pg-%s-%s", var.project_name, var.environment_name)
