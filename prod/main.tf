...
module "key_vault" {
  ...
  environment_name    = local.environment_name
  region_name         = var.region_name
  subnet_ids          = [module.virtual_network.storage_subnet_id]
}
...
module "languagetool" {
  source                       = "../modules/container_apps/languagetool"
  resource_group_name          = module.resource_group.name
  project_name                 = var.project_name
  environment_name             = local.environment_name
...
module "webapp" {
  source                       = "../modules/container_apps/webapp"
  resource_group_name          = module.resource_group.name
  project_name                 = var.project_name
  environment_name             = local.environment_name
  container_app_environment_id = module.container_app_environment.container_app_environment_id
...
module "cloai_service" {
  source                       = "../modules/container_apps/cloai_service"
  resource_group_name          = module.resource_group.name
  project_name                 = var.project_name
  environment_name             = local.environment_name
  container_app_environment_id = module.container_app_environment.container_app_environment_id
...
module "ctk_functions" {
  source                                   = "../modules/container_apps/ctk_functions"
  resource_group_name                      = module.resource_group.name
  project_name                             = var.project_name
  environment_name                         = local.environment_name
  container_app_environment_id             = module.container_app_environment.container_app_environment_id
  ...
  cloai_service_url                        = "http://${module.cloai_service.fqdn}"
  languagetool_url                         = "http://${module.languagetool.fqdn}"
